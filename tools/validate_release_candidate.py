...
def metadata() -> list[str]:
    ...
    errors: list[str] = []
    if plugin.get("name") != "agent-shipproof" or plugin.get("version") != "0.1.4" or plugin.get("license") != "MIT":
...
def run_validation() -> dict[str, Any]:
    ...
    return {"status": "pass" if not errors else "fail", "candidate": "agent-shipproof 0.1.4", "product_revision_sha256": rev(), "checks": {name: "pass" if not values else "fail" for name, values in checks.items()}, "errors": errors}
