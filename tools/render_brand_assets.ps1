...
$manifestFiles = @(Get-ChildItem -LiteralPath $pluginRoot -Recurse -File -Filter 'Logo Generation Manifest 140826.json')
if ($manifestFiles.Count -ne 1) {
    throw "Expected exactly one Logo Generation Manifest 140826.json below $pluginRoot; found $($manifestFiles.Count)."
}
...
try {
    ...
    if ($manifest.source_background_policy -eq 'transparent_source') {
        if ($master.PixelFormat.ToString() -notmatch 'Argb') { throw "Transparent canonical master must retain an alpha channel." }
        ...
        if (@($cornerAlpha | Where-Object { $_ -ne 0 }).Count -gt 0) { throw "Transparent canonical master must have transparent corners; got $($cornerAlpha -join ',')." }
    ...
    foreach ($requiredCropField in @('x', 'y', 'width', 'height')) { if (-not $crop.PSObject.Properties.Name.Contains($requiredCropField)) { throw "Logo source crop is missing '$requiredCropField'." } }
    if ([int]$crop.width -ne [int]$crop.height) { throw "Logo source crop must be square to preserve aspect ratio." }
    if ([int]$crop.x -lt 0 -or [int]$crop.y -lt 0 -or [int]$crop.width -le 0 -or [int]$crop.height -le 0) { throw "Logo source crop has invalid bounds." }
    if (([int]$crop.x + [int]$crop.width) -gt $master.Width -or ([int]$crop.y + [int]$crop.height) -gt $master.Height) { throw "Logo source crop exceeds the immutable master bounds." }
    ...
    $iconBackground = if ($manifest.source_background_policy -eq 'preserve_opaque_source') { [string]$cfg.light_background } else { 'transparent' }
    ...
    [ordered]@{
        status = 'ok'
        source_policy = 'accepted_transparent_derivative_only'
